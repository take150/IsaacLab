...
class randomize_dome_light(ManagerTermBase):
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        """Initialize the term.
        ...
        # convert from radians to degrees
        # light_rotation が数値のタプルの場合、またはタプル内が数値の場合
        if isinstance(light_rotation, (list, tuple)):
            # もし最初の要素自体がリストやタプルなら、各軸毎のタプルと判断
            if isinstance(light_rotation[0], (list, tuple)):
                # ((float, float), (float, float), (float, float)) として各軸ごとに変換
                light_rotation = tuple(
                    tuple(math.degrees(angle) for angle in axis_tuple)
                    for axis_tuple in light_rotation
                )
            else:
                # (float, float) の場合はそのまま一括変換
                light_rotation = tuple(math.degrees(angle) for angle in light_rotation)
        else:
            # 数値が1つだけ渡された場合は全軸同じ値とみなす（ここではシングル値を 3 つに複製）
            angle_deg = math.degrees(light_rotation)
