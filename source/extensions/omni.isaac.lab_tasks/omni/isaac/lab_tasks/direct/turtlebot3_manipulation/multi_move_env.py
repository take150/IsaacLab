...
@configclass
class Turtlebot3MultiMoveEnvCfg(DirectMARLEnvCfg):
    # env
    episode_length_s = 14.4  # 1500 timesteps
    decimation = 2
    possible_agents = ["robot_1", "robot_2"]
    action_spaces = {"robot_1": 7, "robot_2": 7}
    observation_spaces = {"robot_1": 18, "robot_2": 18}
    state_space = -1
    seed = 42
    ...
    # scene
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=2048, env_spacing=5, replicate_physics=True)
...
class Turtlebot3MultiMoveEnv(DirectMARLEnv):
    ...
    cfg: Turtlebot3MultiMoveEnvCfg

    def __init__(self, cfg: Turtlebot3MultiMoveEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        arm_targets = self._robot_1.data.joint_pos[:, self.arm_ids] + self.robot_dof_vel_limits_tensor[self.arm_ids] * self.dt * arm_actions_1
        self.robot_arm_targets_1[:] = torch.clamp(arm_targets, self.robot_dof_lower_limits[self.arm_ids], self.robot_dof_upper_limits[self.arm_ids])

        arm_targets = self._robot_2.data.joint_pos[:, self.arm_ids] + self.robot_dof_vel_limits_tensor[self.arm_ids] * self.dt * arm_actions_2
        self.robot_arm_targets_2[:] = torch.clamp(arm_targets, self.robot_dof_lower_limits[self.arm_ids], self.robot_dof_upper_limits[self.arm_ids])
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        default_cube_state_1[:, :3] += self.reset_root_state_uniform(
            env_ids=env_ids,
            pose_range = {"x": (0.3, 0.5), "y": (0.9, 1.1), "z": (0.0, 0.0)},
        )
        ...
        default_cube_state_2[:, :3] += self.reset_root_state_uniform(
            env_ids=env_ids,
            pose_range = {"x": (0.3, 0.5), "y": (-1.1, -0.9), "z": (0.0, 0.0)},
        )
    ...
    def _compute_rewards(
        ...
        rewards = {
            "robot_1": dist_reward_scale * dis_reward_1 + lift_reward_scale * lift_reward_1 + dist_g_reward_scale * dis_g_reward_01 + dist_g_reward_scale * dis_g_reward_11,
            "robot_2": dist_reward_scale * dis_reward_2 + lift_reward_scale * lift_reward_2 + dist_g_reward_scale * dis_g_reward_02 + dist_g_reward_scale * dis_g_reward_12,
        }
