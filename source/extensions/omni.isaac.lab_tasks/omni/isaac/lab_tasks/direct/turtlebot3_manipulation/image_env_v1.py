...
class Turtlebot3ImageEnvCfg(DirectRLEnvCfg):
    # env
    episode_length_s = 12.01  # 1500 timesteps
    decimation = 2
    state_space = 0
    seed = 42
...
class Turtlebot3ImageEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        default_goal_state = self._goal.data.default_root_state[env_ids].clone()
        default_goal_state[:, :3] += self.scene.env_origins[env_ids]
        default_goal_state[:, :3] += self.reset_root_state_uniform(
            env_ids=env_ids,
            pose_range = {"x": (1.0, 1.2), "y": (-0.2, 0.2), "z": (0.0, 0.0)},
        )
    ...
    def _get_observations(self) -> dict:
        ...
        # image_np = self._camera.data.output["rgb"][0, ...].cpu().numpy().astype(np.uint8)  # RGB形式に変換
        # # image_np = self.scene["camera"].data.output["rgb"][0, ...].cpu().numpy().astype(np.uint8)  # RGB形式に変換
        # image_np = cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR)  # OpenCV用にBGRに変換

        # cv2.namedWindow('Camera Feed', cv2.WINDOW_NORMAL)
        # # cv2.resizeWindow('Camera Feed', 512, 512)
