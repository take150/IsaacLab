...
class Turtlebot3SinglePlaceEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # mask = (self.task_id == 1)
        # col = len(self.arm_ids)
        # sel = actions[mask, col]  # shape確認用
        # actions[mask, col] = torch.empty_like(sel).uniform_(-1.0, -0.95)
        # actions[self.task_id==1, len(self.arm_ids)] = -1.0
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        return self._compute_rewards(
            ...
            self.cfg.gripper_penalty_scale,
        )
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        positions_goal, orientations_goal= self.reset_root_state_uniform(
            ...
            pose_range={"x": (0.5, 0.6), "y": (-0.05, 0.05), "z": (0.00, 0.00), "yaw": (0.0, 0.0)},
            avoid_radius=0.2
        )
        ...
        positions_delta, orientations_delta= self.reset_root_state_uniform(
            ...
            pose_range={"x": (0.25, 0.4), "y": (-0.05, 0.05), "z": (0.01, 0.01), "yaw": (0.0, 0.0)},
            avoid_radius=0.2
        )
    ...
    def _compute_rewards(
        ...
        success_reach_mask = (
            is_reached &
            (self.reach != -1) &
            ((self.episode_length_buf - self.reach) >= 8) &
            self.log
        )

        self.success_time[success_reach_mask] = self.episode_length_buf[success_reach_mask]
        self.log[success_reach_mask] = False
        ...
        gripper_open_penalty = (self.curr_actions[:, len(self.arm_ids)] >= 0).float()

        reward[task_1] += (
            dist_g_reward_scale * dis_g_reward[task_1] * is_lifted[task_1]
            + drop_penalty_scale * is_dropped[task_1].float()
            - 0.2 * on_goal[task_1].float()
            + drop_penalty_scale * gripper_open_penalty[task_1]
        )
        # reward[task_1] = 0
        ...
        self.task_id[failed_lift_mask & task_1] = 0
