...
class Turtlebot3SinglePlaceEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if not self.reward_delay:
            positions_goal, orientations_goal= self.reset_root_state_uniform(
                ...
                pose_range={"x": (2.0, 2.0), "y": (-0.05, 0.05), "z": (0.001, 0.001), "yaw": (0.0, 0.0)},
                avoid_radius=0.2
            )
        else:
            positions_goal, orientations_goal= self.reset_root_state_uniform(
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        goal_pos_e, goal_rot_e = math_utils.subtract_frame_transforms(
            self.ee_pos, self.ee_rot, goal_pos, goal_rot
        )
        ...
        self.goal_be= torch.cat((goal_pos_b, goal_rot_b, goal_pos_e, goal_rot_e), dim=1)
